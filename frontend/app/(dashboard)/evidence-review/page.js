...
export default function EvidenceReviewPage() {
  ...
  const { data = [] } = useQuery({ queryKey: ["jobs-evidence"], queryFn: async () => (await api.get("/research-jobs")).data.jobs });
      ...
      <CardContent className="overflow-auto p-0">
        <table className="w-full min-w-[760px] text-left text-sm">
          ...
          <tbody>{rows.length ? rows.map(e => <tr key={e._id} className="border-b border-border/70">
            ...
            <td className="flex gap-2 p-4"><Button variant="secondary" onClick={() => review.mutate({ id: e._id, status: "approved" })}>Approve</Button><Button variant="danger" onClick={() => review.mutate({ id: e._id, status: "rejected" })}>Reject</Button></td>
